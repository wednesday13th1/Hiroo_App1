...
final class MapViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    private func config(for school: School) -> MapConfig {
        ...
        case .hiroo:
            return MapConfig(
                title: NSLocalizedString("maphiroo", comment: ""),
                subtitle: "Hiroo Gakuen",
                coordinate: CLLocationCoordinate2D(latitude: 35.6514, longitude: 139.7209),
                url: URL(string: "https://world.jorudan.co.jp/mln/en/")!,
                metroText: NSLocalizedString("metrotext", comment: ""),
                busText: NSLocalizedString("bushg", comment: "")
            )
        case .koishikawa:
            return MapConfig(
                title: NSLocalizedString("mapkoishikawa", comment: ""),
                ...
                metroText: NSLocalizedString("toeitext", comment: ""),
                busText: NSLocalizedString("bushgk", comment: "")
            )
        }
    }
...
//    }
    ...
    private func applyConfig(for school: School) {
       ...
       // URL ラベル（“乗車案内” 部分だけリンク風に）
        let baseText = NSLocalizedString("bt", comment: "")
        let attributed = NSMutableAttributedString(string: baseText)
        if let range = baseText.range(of: NSLocalizedString("bt1", comment: "")) {
